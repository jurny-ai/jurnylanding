...
const FAQ = () => {
  return (
    <section id="faq" className="py-16 bg-background" aria-labelledby="faq-title">
      <div className="container mx-auto px-4">
        <div className="max-w-7xl mx-auto">
          <div className="bg-secondary rounded-3xl p-4 sm:p-8 md:p-12">
            ...
            <div className="rounded-2xl sm:rounded-3xl bg-card border border-primary/15 px-4 py-2 sm:px-7 sm:py-4">
              <Accordion type="single" collapsible>
                {faqs.map((faq) => (
                  <AccordionItem key={faq.question} value={faq.question}>
                    <AccordionTrigger className="text-left text-base font-semibold leading-snug text-foreground hover:no-underline sm:text-lg">
                      {faq.question}
                    </AccordionTrigger>
                    <AccordionContent className="pr-8 text-sm leading-relaxed text-foreground/65 sm:text-base">
                      {faq.answer}
                    </AccordionContent>
                  </AccordionItem>
                ))}
